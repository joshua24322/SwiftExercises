...
//                           "For Kee Restaurant", "Ps's Atelier", "Bourkd Street Bakery", "Haigh's Chocolate", "Palomino Espresso", "Upstate", "Traif", "Graham Avenue Meats And Deli", "Waffle & Wolf", "Five Leaves", "Cafe Lore", "Confessional", "Barrafina", "Donostia", "Royal Oak", "CASK Pub and Kitchen"]
//
...
//    //用此布林陣列來儲存勾選過的餐廳，初始化為false
    
    
    // MARK: - Restaurants's Properties
    ...
    override func viewDidLoad() {
        ...
        navigationController?.navigationBar.prefersLargeTitles = true // 啟用大標題

        // MARK: - 讓導覽列變透明
        // 設定背景圖片與陰影圖片為一張「空白」圖片(也就是UIImage())
             ...
             */
        {
            navigationController?.navigationBar.largeTitleTextAttributes = [
                NSAttributedStringKey.foregroundColor: UIColor(displayP3Red: 231.0/255.0, green: 76.0/255.0, blue: 60.0/255.0, alpha: 1.0), NSAttributedStringKey.font: customFont
            ]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // 將dequeueReusableCell回傳的物件「轉換」為RestaurantTableViewCell
        
        // 使用自訂類別「RestaurantTableViewCell」的屬性，設定Cell...
        cell.nameLabel.text = restaurants[indexPath.row].name
        cell.locationLabel.text = restaurants[indexPath.row].location
        cell.typeLabel.text = restaurants[indexPath.row].type
        cell.thumbnailImageView.image = UIImage(named: restaurants[indexPath.row].image)
        cell.accessoryViewImage.isHidden = restaurants[indexPath.row].isVisited ? false : true
        
        return cell
        
        //cell.accessoryType = restaurantIsVisited[indexPath.row] ? .checkmark : .none
        // 簡單的 if else 判斷是可以用三元條件判斷式寫法更有簡潔
        // 檢查這個cell的餐廳是否被勾選，如果restaurantIsVisited[indexPath.row]是true，就會在Cell顯示打勾符號，否則就不顯示
    }
...
//        
//        //加入打電話動作
//        let callAction = UIAlertAction(title: "Call " + "123-000-\(indexPath.row + 1)", style: .default) { (action:UIAlertAction!) in
...
//            /*
//             Cell的右邊是預留給輔助視圖(Accessory View)用的。
//             有4種內建的輔助視圖，包含「揭示指示符號」(discloure indicator)、「細節揭示符號」(detail discloure button)、「打勾符號」(checkmark)以及「細節圖示」(detail)
//             */
//            //使用indexpath來取得所選取的表格cell，以及所選cell的索引值
//            //let cell = tableView.cellForRow(at: indexPath)
//            
//            //更新cell的accessoryType屬性為打勾符號
//            //cell?.accessoryType = .checkmark
//            
//            // 更新被打勾的cell，並且從false改為true
//            //self.restaurantIsVisited[indexPath.row] = true
//            
////            10.9 Solution to Exercise #2
//            
//            let cell = tableView.cellForRow(at: indexPath) as! RestaurantTableViewCell
//            
//            self.restaurantIsVisited[indexPath.row] = (self.restaurantIsVisited[indexPath.row]) ? false : true
//            
//            cell.accessoryViewImage.isHidden = self.restaurantIsVisited[indexPath.row] ? false : true
//        }
//        
...
//                //設定sourceRect的屬性為cell的邊界，因此彈出的原點來自cell的中心
//            }
//        }
//        
//        //呈現選單，這行程式碼iOS會檢查裝置是iPhone或是iPad
//        present(optionMenu, animated: true, completion: nil)
//        /*
//         若裝置是iPhone的話，提示控制器會以強制回應對話視窗來呈現，
//         若裝置是iPad的話，提示控制器會使用儲存在popoverPresentationController屬性中的UIPopoverPresentationController，
//         是因為在iPad上不會以回應對話視窗的方式呈現提示控制器，而是使用彈出(popover)的樣式
//         */
//        
//        //取消列的選取
...
//    }
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        //tableView.reloadData() 更新視圖，重新載入UITableView
        tableView.deleteRows(at: [indexPath], with: .fade)
        //由於只需要刪除一列，呼叫deleteRows(at:with:)方法從表格視圖中移除特定列(或多列)，並取代reloadData()
    ...
    */
    ...
    //用UIContextualAction滑動帶出其他動作
    /*
    在iOS11中，導入兩個新方法來處理表格視圖cell的滑動動作：
     
    ＊tableView(_:leadingSwipeActionConfigurationForRowAt:)
    ＊tableView(_:trailingSwipeActionConfigurationForRowAt:)
     
    這兩個方法被定義在UITableViewDelegate。
    第一個方法是處理「向右滑動」，
    第二個方法是處理「向左滑動」。
    */
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let shareAction = UIContextualAction(style: .normal, title: "Share") { (action, sourceView, completionHandler) in
            ...
            /*
             由於iPad必須以彈出方式(popover)來呈現視圖控制器，
             必須設定 popoverPresentationController的來源視圖。
             */
        ...
        /*
         UIContextualAction類別提供開發者幾個屬性來自訂樣式，
         包括背景顏色與圖示
         */
    ...
    //            11.12 Solution to Exercise #2
    override func tableView(_ tableView: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        
        let tickAction = UIContextualAction(style: .normal, title: "Check-in") { (action, sourceView, compltionHandler) in
            let cell = tableView.cellForRow(at: indexPath) as! RestaurantTableViewCell
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showRestaurantDetail" { // 檢查Segue的識別碼
            if let indexPath = tableView.indexPathForSelectedRow {
            //從tableView.indexPathForSelectedRow取得所選的列
                let destinationController = segue.destination as! RestaurantDetailViewController
                //用segue.destination來取得轉場的目標，找到轉換場景的目的地
                //再用as!向下轉型找到真正目的地的ViewController
